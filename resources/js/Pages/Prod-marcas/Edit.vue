    ...
    <AuthenticatedLayout>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 border-b border-gray-200">
                <form @submit.prevent="$event => form.patch(route('prod-marcas.update', prodmarca))" >
                  <div class="mt-4 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-3">
                        <InputLabel for="supervisado" value="Marca Supervisada?" />
                        <div class="mt-2">
                            <select id="supervisado" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" v-model="form.supervisado" required>
...
const form = useForm({
    name: props.prodmarca.name,
    supervisado: props.prodmarca.supervisado,
    descripcion: props.prodmarca.descripcion,
    empresa_id:props.prodmarca.empresa_id
});
</script>